...
var enumToStr = map[MyEnum]string{
	One:   "map",
	Two:   "reduce",
	Three: "wait",
	Four:  "exit",
...
func (e MyEnum) String() string {
	if s, ok := enumToStr[e]; ok {
		return s
	}
	return fmt.Sprintf("MyEnum(%d)", e)
}

func taskTypeToString(enumVal MyEnum) string {
	if s, ok := enumToStr[enumVal]; ok {
		return s
	}
	return fmt.Sprintf("invalid MyEnum value: %d", enumVal)
}
